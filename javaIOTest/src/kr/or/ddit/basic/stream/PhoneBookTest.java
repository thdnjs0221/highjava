 ...
 *  	(저장파일명: 'phoneBookData.dat'로 한다.) Map에 있는걸 파일에 저장
 *  2) 프로그램이 시작될 때 저장된 파일이 있으면 그 데이터를 읽어와서 사용한다.
 *  	(읽어온 데이터를 Map에 저장한다.) 자동으로 읽어와서 Map에 저장
 *  3) 프로그램이 종료할 때 Map의 데이터가 추가, 수정, 삭제 등으로 변경되었으면
 *  	저장한 후에 종료되도록 한다.
 *  
 *  
 *  
...
public class PhoneBookTest {
	...
	private void save() {
		...
		DataInputStream data  = new DataInputStream(phoneBookMap.get(key));
	}
