...
//			// 형식) SqlSessionFactory객체.openSession(논리값) // '논리값'이 true이면 AutoCommit이활성화되고 //
...
//				// 작업이 성공하면AutoCommit이 비활성화된 상태이기 때문에 직접 commit을 실행해야 한다
//				session.commit();
//			} else {
...
//		System.out.println("---------------------------------------------------------");
		...
		// 2-2. update 연습 System.out.println("update 시작...");
...
//			session = sqlSessionFactory.openSession();
//
//			// Sqlsession객체를 이용하여 처리한다 //응답 결과가 여러개일 경우에는 selectList()메서드를 사용하는데 이 메서드는
...
//		SqlSession session = null;
//
//		try { // SqlSession객체 생성 session = sqlSessionFactory.openSession();
//
//			// SqlSession객체를 이용하여 실행한다
//			// 응답 결과가 한 개일 경우에는 'selectOne()'메서드를 사용한다
//			// 형식:SqlSession객체.selectOne("namespace속성값.실행할id속성값", 파라미터클래스)
//			// 반환값: sql문을 작성한태그의 resultType에 지정한 객체가 반환된다
