...
public class CookieAdd extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//Cookie 저장하는 방법
		//1. Cookie객체를 생성한다 ==>'쿠키이름'과 '쿠키값'을 *문자열*로 지정하여 생성한다.
		//		형식) Cookie cookie변수 = new Cookie("쿠키이름","쿠키값");
		//		    ==> 쿠기값에 한글을 사용할 경우에는 URLEncoder객체로 인코딩한 후 저장한다	
		//			==> 하나의 쿠키는 4KB(4096byte)까지 저장 가능!!
		//			==> 하나의 도메인 당 20개(총 300개)까지 저장 가능
		
		Cookie naemCookie = new Cookie("name", URLEncoder.encode("홍길동","utf-8"));
		Cookie ageCookie= new Cookie("age", "20"); //숫자도 문자열로 저장하기 
		...
		//							  ==> 생략하면 -1 값을 갖는다.
		//							  ==> (-1: 브라우저가 종료될 때까지 유지된다. /  0: 즉시 삭제된다.)
		
		//cookie변수.setDomain("적용도메인명"); ==> 예) ".ddit.or.kr"--> www.ddit.or.kr /mail.ddit.or.kr/cafe.ddit.or.kr 된다 
